...
const filteredReviews = computed(() => {
  ...
  return allReviews.value.filter(
    (review) => review.media_type === filterType.value
  );
});
...
async function deleteReview(review: CombinedReview, event: Event) {
  ...
  if (
    !confirm(
      `Are you sure you want to delete your review for "${review.title}"?`
    )
  ) {
    return;
  }
...
<template>
  <div
    class="bg-white/5 dark:bg-gray-900/30 rounded-xl shadow-md p-6 border border-gray-100 dark:border-gray-700"
  >
    <div class="mb-6">
      ...
      <p class="text-gray-600 dark:text-gray-400 text-sm mt-1">
        Your ratings and reviews for movies and TV shows
      </p>
    </div>
      ...
      >
        ...
        <div class="flex items-start gap-4">
          ...
          >
            ...
            <div
              v-else
              class="w-full h-full flex items-center justify-center text-gray-500 dark:text-gray-400 text-xs"
            >
              No Image
            </div>
          </div>
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="mb-3">
              ...
              >
                <span
                  class="px-2 py-0.5 bg-gray-200 dark:bg-gray-700 rounded capitalize"
                >
                  {{ review.media_type }}
                </span>
            ...
            <div class="relative">
              <p
                :class="[
                  ...
                  shouldShowReadMore(review.review_text)
                    ? 'line-clamp-3'
                    : '',
                ]"
              >
            ...
            <div class="flex justify-end mt-4">
              <div
                class="flex items-center gap-1 bg-white dark:bg-gray-700 px-3 py-2 rounded-lg shadow-sm"
              >
                <span v-for="i in 10" :key="i" class="text-base">
                  {{ i <= review.rating ? "⭐" : "☆" }}
                </span>
                <span
                  class="ml-2 text-sm font-bold text-gray-900 dark:text-gray-100"
                >
                  {{ review.rating }}/10
                </span>
              </div>
