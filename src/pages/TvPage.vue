<template>
  <BaseNavBar />
  <div class="min-h-screen relative overflow-hidden bg-gray-900 text-[#eaf0ff]">
    
    <div class="relative overflow-hidden border-b border-gray-800 bg-gray-800">
      <div class="mx-auto px-4 py-14">
        ...
        <p class="text-lg text-gray-400">
          Explore trending, latest, upcoming, and top-rated films — handpicked & comfy to browse.
        </p>
          ...
          <button
            ...
            :class="[
              ...
              activeCategory === cat.id
                ? 'bg-blue-700 text-white shadow-md translate-y-[-0.5rem]'
    ...
    <main class="mx-auto p-4">
      ...
      <div v-if="loading" class="flex flex-col items-center justify-center py-20">
        <div class="w-16 h-16 rounded-full border-4 border-t-blue-600 border-l-gray-600 border-r-gray-700 border-b-gray-700 animate-spin mb-4"></div>
        <p class="text-gray-400 text-lg">Loading amazing movies...</p>
      </div>

      <!-- Error State -->
      <div v-else-if="error" class="my-8 p-6 rounded-2xl bg-red-900 border border-red-800 text-red-400">
      ...
      <!-- Empty State -->
      <div v-else-if="movies.length === 0" class="my-8 p-12 rounded-2xl bg-gray-800 border border-gray-700 text-center text-gray-400">
      ...
      <!-- Movies Grid -->
      <div v-else class="mt-8 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        <router-link
        ...
        >
          <div class="relative aspect-[2/3] bg-gray-900">
            ...
            <div v-if="movie.rating" class="absolute top-2 right-2 bg-black bg-opacity-75 px-2.5 py-1 rounded-lg flex items-center gap-1 border border-gray-700 text-yellow-400">
          ...
          <div class="p-3">
            <h3 class="font-semibold text-sm line-clamp-2 group-hover:text-blue-500 transition-colors" :title="movie.title">{{ movie.title }}</h3>
            <div class="flex items-center justify-between mt-2 text-xs">
              <span class="text-gray-400">{{ movie.year }}</span>
              <span v-if="movie.voteCount" class="text-gray-500">{{ formatVotes(movie.voteCount) }} votes</span>
            </div>
          </div>
        </router-link>
      </div>
      ...
      <div v-if="!loading && movies.length > 0" class="flex justify-center mt-10 mb-6">
        ...
        >
          <template v-if="!loadingMore">
            <svg class="w-4 h-4 stroke-current opacity-95 translate-y-[0.5px]" aria-hidden="true" viewBox="0 0 24 24" fill="none">
              <path d="M12 4v12M8 12l4 4 4-4" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
            <span>Load more</span>
          </template>
          <template v-else>
            <span class="w-3.5 h-3.5 rounded-full border-2 border-white/20 border-t-white animate-spin inline-block box-border mr-2"></span>
            <span>Loading</span>
          </template>
        </button>
      </div>
