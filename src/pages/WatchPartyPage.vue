...
<template>
  ...
  <div class="max-w-4xl mx-auto p-8">
    <div
      v-if="showToast"
      class="fixed top-6 left-1/2 -translate-x-1/2 bg-green-600 px-6 py-3 rounded shadow-lg z-50"
    >
      {{ toastMessage }}
    </div>
    ...
    <div class="mb-10 border-b pb-6">
      ...
      <p class="mb-4 text-gray-600">
        Join or create watch parties with your friends!
      </p>
      <button
        @click="openModal"
        class="create-party-btn bg-blue-500 hover:bg-blue-600 py-2 px-4 rounded"
      >
        + Create New Party
      </button>
    </div>
    ...
    <section class="mb-10">
      ...
      <div
        v-if="publicParties.length === 0"
        class="bg-gray-50 p-5 rounded shadow text-gray-500"
      >
        No public parties available at the moment.
      </div>
      <div v-else class="space-y-4">
        <div
          v-for="party in publicParties"
          :key="party.id"
          class="bg-yellow-50 p-4 rounded shadow flex flex-col md:flex-row md:items-center md:justify-between border border-yellow-200"
        >
          <div>
            <div class="flex items-center gap-2 mb-1">
              <h3 class="font-semibold text-lg text-yellow-900">
                {{ party.name }}
              </h3>
              <span
                class="text-xs font-bold rounded bg-green-100 text-green-700 px-2 py-1"
                >Public</span
              >
            </div>
          ...
          <button
            @click="joinPublicParty(party)"
            class="mt-2 md:mt-0 bg-yellow-400 hover:bg-yellow-500 px-4 py-2 rounded font-semibold"
          >
            Join
          </button>
        </div>
    ...
    <section class="mb-10">
      ...
      <div v-else class="space-y-4">
        <div
          ...
          :key="party.id"
          class="bg-white p-4 rounded shadow flex flex-col md:flex-row md:items-center md:justify-between"
        >
          <div>
            <div class="flex items-center gap-2 mb-1">
              ...
              <span
                v-else
                class="text-xs font-bold rounded bg-gray-200 text-gray-600 px-2 py-1"
              >
                Private
              </span>
            </div>
            ...
            <p v-if="party.invitees.length" class="text-gray-700 mt-2">
              Invitees:
              <span v-for="user in party.invitees" :key="user" class="mr-1"
                >@{{ user }}</span
              >
            </p>
          </div>
          <div class="flex flex-col gap-2 mt-3 md:mt-0">
            <button
              @click="copyPartyLink(party.id)"
              class="bg-gray-100 hover:bg-gray-200 text-sm px-4 py-1 rounded border border-gray-300"
            >
              Copy Party Link
            </button>
            <button
              v-if="isPartyNow(party.date) && liveChatPartyId !== party.id"
              @click="joinPartyLive(party.id)"
      ...
      <!-- Live Chat Popover -->
      <div
        v-if="liveChatPartyId"
        class="fixed bottom-8 right-8 z-40 w-[500px] h-[420px] max-w-full bg-white rounded shadow-lg border border-green-500 flex flex-col"
      >
        <div
          class="flex justify-between items-center px-4 py-2 border-b bg-green-50"
        >
        ...
        <div
          ...
          class="p-4 flex-1 overflow-y-auto flex flex-col"
        >
          <div
            v-if="chatMessages.length === 0"
            class="text-gray-400 text-sm flex-1"
          >
            No messages yet.
          </div>
          <div
            ...
            :key="i"
            class="my-2 flex flex-col items-end"
          >
            <span class="font-semibold text-blue-700 mb-1">{{ msg.user }}</span>
            <span
              class="bg-green-100 px-4 py-2 rounded-xl max-w-[75%] text-gray-900 text-right shadow"
              >{{ msg.text }}</span
            >
          </div>
        </div>
        ...
        >
          <input
            v-model="chatInput"
            class="border rounded flex-1 px-2 py-2"
            placeholder="Type a message..."
            autocomplete="off"
          />
          <button type="submit" class="bg-green-500 rounded px-4 py-2">
            Send
          </button>
        </form>
      </div>
      ...
      <form @submit.prevent="submitParty">
        ...
        <div class="mb-3">
          <label class="block mb-1 font-medium"
            >Invite friends (type <b>@username</b> or paste/copy link):</label
          >
          <input
            v-model="inviteeText"
          ...
          <div class="mt-1 flex gap-2">
            <button
              type="button"
              @click="copyPartyLink(Date.now(), 'Invite link copied!')"
              class="text-blue-600 underline"
            >
              Copy Invite Link
            </button>
            <span class="text-gray-500 text-xs"
              >(Or share above link in chat)</span
            >
          </div>
        </div>
