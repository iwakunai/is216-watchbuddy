  ...
  >
    <div class="mx-auto p-4 text-[#eaf0ff]">
      <div v-if="loading" class="py-20 flex justify-center items-center">
        <div class="w-12 h-12 mr-4 border-4 border-t-[#6b6bff] border-l-[rgba(255,255,255,0.06)] border-r-[rgba(255,255,255,0.04)] border-b-[rgba(255,255,255,0.02)] rounded-full animate-spin"></div>
      ...
      <div v-else class="grid gap-8 lg:grid-cols-3">
        <!-- Poster Column -->
        <div class="col-span-3 md:col-span-1 rounded-xl overflow-hidden shadow-lg bg-gradient-to-b from-white/5 to-black/20">
          <img
          ...
          <div class="flex mt-3 items-center justify-center gap-4">
            <div class="w-16 h-16 rounded-xl flex flex-col items-center justify-center border border-white/5 bg-gradient-to-b from-white/5 to-black/30 shadow-lg">
          ...
          <div class="mt-2 p-3 text-[#98a1b3]">
            <div class="text-base">
              <strong class="text-white">Original Title:</strong> {{ show.original_name || "-" }}
            </div>
            <div v-if="show.tagline" class="mt-2 italic text-base text-[#a6b0c6]">
              "{{ show.tagline }}"
            </div>
            ...
            <div class="mt-2 text-base text-[#a6b0c6]">
              <strong class="text-white">Status:</strong> {{ show.status || "-" }}
            </div>
          </div>
        </div>
        ...
        <div class="col-span-3 md:col-span-2 gap-[2rem]">
          <div class="flex flex-col items-center gap-[4rem] h-min">
            <div class="flex-1 items-center">
              <h1 class="text-3xl md:text-4xl font-extrabold text-white text-center leading-tight">
                {{ show.name }}
                <span class="text-base font-medium text-[#b6c0d6]">({{ firstAirYear }})</span>
              </h1>
              ...
              <div class="mt-3 flex flex-wrap items-center justify-center gap-3 text-sm text-[#98a1b3]">
                <span class="px-2 py-1 rounded-md border border-white/5 bg-white/[0.03] text-center">
          ...
          <div class="grid md:grid-cols-3 gap-6 mt-6">
            <div class="md:col-span-3">
              <div class="p-5 rounded-lg border border-white/10 bg-gradient-to-b from-white/5 to-black/20">
                ...
                <div class="mt-4 grid gap-2 sm:grid-cols-2">
                  <div>
                    ...
                    <div class="mt-1 font-semibold text-white text-base">
                      ...
                      >
                        {{ creator.name }}<span v-if="idx < show.created_by.length - 1">, </span>
                      </span>
                      <span v-if="!show.created_by || show.created_by.length === 0">-</span>
                    </div>
                  </div>
                  <div>
          ...
          <div class="mt-4 grid md:grid-cols-2 gap-6">
            <div>
              ...
              <div class="grid gap-3 text-base text-[#98a1b3]">
                ...
                <div class="grid grid-cols-2">
                  <div class="font-semibold text-white text-left">Original Language:</div>
                  <span class="text-left">{{ show.original_language?.toUpperCase() || "-" }}</span>
                </div>
                <div v-if="show.production_companies?.length" class="grid grid-cols-2">
                  <div class="font-semibold text-white text-left">Production:</div>
        ...
        <div v-if="allCast.length > 0" class="mt-12 col-span-3">
          <h2 class="text-2xl font-bold mb-6 bg-gradient-to-r from-white to-[#cfd8ff] text-transparent bg-clip-text">
            Cast
          </h2>
          <div class="grid grid-cols-[repeat(auto-fill,minmax(250px,1fr))] gap-4">
            ...
            >
              <div class="flex items-center gap-4">
                <div class="w-[60px] h-[60px] rounded-md overflow-hidden bg-white/5 flex-shrink-0">
                  <img
        ...
        <div v-if="similarShows.length > 0" class="col-span-3 mt-12 mb-8">
          ...
          <div class="grid grid-cols-[repeat(auto-fill,minmax(160px,1fr))] gap-4">
            ...
            >
              ...
              <div class="p-3">
                <h3 class="font-semibold text-sm text-white truncate group-hover:text-[#6b6bff] transition-colors">
                  {{ similar.name }}
                </h3>
                <div class="flex items-center gap-2 mt-1 text-xs text-[#98a1b3]">
                  <span v-if="similar.vote_average">⭐ {{ similar.vote_average.toFixed(1) }}</span>
                  <span class="text-[#6b7280]">
                    {{ similar.first_air_date ? new Date(similar.first_air_date).getFullYear() : "" }}
                  </span>
                </div>
              </div>
            </div>
...
async function loadWatchlistStatus() {
  ...
  try {
    const status = await getWatchlistStatus(user.value.id, Number(showId.value), 'tv')
...
async function fetchAll() {
  ...
  if (!TMDB_API_KEY) {
    error.value = "TMDB API key not configured. Set VITE_TMDB_API_KEY in your .env file";
    loading.value = false;
    return;
  }
