<template>
  ...
  <div
    class="h-screen flex flex-col bg-slate-900/50 text-white overflow-hidden"
  >
    <header
      class="flex items-center justify-between px-4 sm:px-6 py-4 bg-slate-800/80 backdrop-blur-sm border-b border-slate-700/50 flex-shrink-0 shadow-lg z-20"
    >
      <button
        @click="goBack"
        class="flex items-center gap-2 text-sm text-gray-200 hover:text-white bg-slate-700/50 hover:bg-slate-700/80 border border-slate-600/50 px-3 py-2 rounded-full transition-all"
      >
        <svg
          class="w-5 h-5"
          fill="none"
        ...
        <span class="hidden sm:block">Back</span></button
      >>form
      ...
      <div class="w-[88px] flex justify-end">
        <button
          ...
          class="md:hidden p-2 rounded-full hover:bg-slate-700/50"
        >
          <svg
            class="w-6 h-6"
            fill="none"
    ...
    <div class="flex flex-1 overflow-hidden z-0">
      ...
      <aside
        class="fixed md:static top-0 left-0 h-full w-3/4 max-w-[300px] md:w-1/4 md:min-w-[200px] bg-slate-800/80 backdrop-blur-sm border-r border-slate-700/50 p-4 overflow-y-auto flex flex-col z-40 transition-transform duration-300 ease-in-out -translate-x-full md:translate-x-0"
        :class="{ 'translate-x-0': isSidebarOpen }"
      >
        <div
          class="flex items-center justify-between md:hidden pb-4 mb-4 border-b border-slate-700/50"
        >
          ...
          <button
            @click="isSidebarOpen = false"
            class="p-1 rounded-full hover:bg-slate-700/50"
          >
            <svg
              class="w-6 h-6"
              fill="none"
        ...
        <h3
          class="text-sm font-semibold mb-4 text-gray-300 md:flex items-center gap-2 flex-shrink-0 hidden"
        >
          <svg
            class="w-5 h-5"
            fill="none"
          ...
          >
            <path
              ...
              stroke-linejoin="round"
              stroke-width="2"
              d="M17 20h5v-2a3 3 0 00-5.356-2.122M17 20v-2m0 2H5.121a2 2 0 01-1.414-.586l-3-3a2 2 0 010-2.828l3-3a2 2 0 011.414-.586H17m0 0a3 3 0 00-5.356-2.122M17 20h5v-2a3 3 0 00-5.356-2.122M17 20v-2m0 2H5.121a2 2 0 01-1.414-.586l-3-3a2 2 0 010-2.828l3-3a2 2 0 011.414-.586H17m0 0a3 3 0 00-5.356-2.122"
            ></path>
          </svg>
          In Room ({{ users.length }})
        </h3>
        <ul class="space-y-1 flex-1 overflow-y-auto">
          ...
          >
            <div class="flex items-center gap-3">
              <div class="relative">
                <div
                  class="w-8 h-8 rounded-full bg-purple-600 flex items-center justify-center text-sm font-semibold"
                >
                  <img
                ...
                <div
                  class="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-slate-800"
                  :class="[
                    user.status === 'synced' ? 'bg-green-500' : 'bg-yellow-500',
                  ]"
                ></div>
              </div>
              <span>{{ user.name }}</span>
      ...
      <section class="flex-1 flex flex-col overflow-hidden bg-black/20">
        <div class="px-6 py-4 flex-shrink-0 z-0">
          <div
            class="max-w-2xl mx-auto p-4 bg-slate-800/80 backdrop-blur-sm border border-slate-700/50 rounded-xl shadow-lg"
          >
            ...
            <template v-else>
              <div class="text-center">
                ...
                <span
                  class="text-6xl font-bold text-purple-400 animate-pulse"
                  >{{ startCountdown }}</span
                >
              </div>
            </template>
        ...
        >
          ...
          >
            <div
              class="w-10 h-10 rounded-full bg-slate-700 flex items-center justify-center text-sm font-semibold flex-shrink-0"
            >
              <img
        ...
        <div
          class="p-4 border-t border-slate-700/50 bg-slate-900 flex items-center gap-3 flex-shrink-0 shadow-2xl"
        >
          <input
            v-model="newMessage"
            ...
            class="flex-1 bg-slate-700 rounded-lg px-4 py-2 text-sm outline-none text-white placeholder-gray-400 focus:ring-2 focus:ring-purple-500 border border-transparent focus:border-purple-500"
          />
          <button
            @click="handleSendMessage"
            class="p-2 bg-purple-600 hover:bg-purple-700 rounded-lg text-sm font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-purple-500"
